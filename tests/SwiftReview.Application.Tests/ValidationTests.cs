...
public sealed class ValidationTests
{
    ...
    [Fact]
    public async Task ReviewAndUndo_RejectInvalidLevelVersionAndReviewId()
    {
        ...
        var reject = await new RejectReviewValidator().ValidateAsync(new RejectReviewRequest(1, "version", ""), TestContext.Current.CancellationToken);
        ...
        Assert.Contains(start.Errors, x => x.PropertyName == "Level"); Assert.Contains(start.Errors, x => x.PropertyName == "RowVersion");
        Assert.Contains(approve.Errors, x => x.PropertyName == "Level"); Assert.Contains(approve.Errors, x => x.PropertyName == "RowVersion");
        Assert.Contains(reject.Errors, x => x.PropertyName == "Comment");
        Assert.Contains(undo.Errors, x => x.PropertyName == "ReviewId"); Assert.Contains(undo.Errors, x => x.PropertyName == "RowVersion");
    }
}
