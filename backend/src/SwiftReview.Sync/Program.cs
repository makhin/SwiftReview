...
internal static class Program
{
    public static int Main()
    {
        try
        {
            var connectionString = ConfigurationManager.ConnectionStrings["SwiftReview"]?.ConnectionString;
            if (string.IsNullOrWhiteSpace(connectionString))
                throw new ConfigurationErrorsException("Connection string 'SwiftReview' is required.");
