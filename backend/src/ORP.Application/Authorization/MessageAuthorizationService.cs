...
public sealed class MessageAuthorizationService(IORPStore store, IUserAccessService users,
    ICurrentUser current, ICorrelationContext correlation, ILogger<MessageAuthorizationService> logger)
{
    public async Task<UserAccess> RequireAsync(long messageId, string permissionName, CancellationToken ct,
        int? reviewLevel = null, MessageActionOwnership ownershipRequirement = MessageActionOwnership.None)
    {
        ...
        var access = await users.GetByIdAsync(currentId, ct) ?? throw new UnauthorizedAccessException();
        if (access.IsGlobalAdministrator) return access;
        var permission = access.HasPermission(permissionName, source.BranchId, source.DepartmentId);
        var branch = access.CanAccess(source.BranchId, source.DepartmentId);
