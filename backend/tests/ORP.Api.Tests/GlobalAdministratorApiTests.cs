...
public sealed class GlobalAdministratorApiTests : IAsyncLifetime
{
    ...
    public async Task WithoutRoles_SeesEveryScopeReferenceAndAudit()
    {
        ...
        var candidates = await admin.GetFromJsonAsync<AssignmentCandidateDto[]>($"/api/messages/{factory.MessageId(1)}/assignment-candidates", Ct);
        Assert.Contains(candidates!, c => c.Id == factory.UserId("admin"));
    ...
    public async Task CanActOnAnotherUsersActiveReview_AndAuditRecordsActualActor(string action, AuditEventType eventType)
    {
        ...
        var started = await reviewer.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/start", new { level = 1 }, Ct);
        var reviewId = (await started.Content.ReadFromJsonAsync<StartReviewResponse>(Ct))!.ReviewId;
        var resumed = await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/start", new { level = 1 }, Ct);
        ...
        (await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/{action}", new { reviewId = await factory.LatestReviewIdAsync(factory.MessageId(1)), level = 1 }, Ct)).EnsureSuccessStatusCode();
        using var scope = factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<ORPDbContext>();
        var audit = await db.AuditEvents.SingleAsync(e => e.MessageId == factory.MessageId(1) && e.EventType == eventType, Ct);
        Assert.Equal(factory.UserId("admin"), audit.UserId);
        Assert.Equal(reviewId, audit.ReviewId);
        Assert.Equal(factory.UserId("amelia.hart"), (await db.Reviews.SingleAsync(r => r.Id == reviewId, Ct)).ReviewerId);
        if (action == "approve")
        {
            (await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/undo", new { reviewId }, Ct)).EnsureSuccessStatusCode();
            Assert.Equal(factory.UserId("admin"), (await db.AuditEvents.SingleAsync(e => e.MessageId == factory.MessageId(1) && e.EventType == AuditEventType.ConfirmationUndone, Ct)).UserId);
        }
        if (action == "cancel")
        {
            (await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/start", new { level = 1 }, Ct)).EnsureSuccessStatusCode();
            Assert.Equal(factory.UserId("admin"), (await db.Messages.AsNoTracking().SingleAsync(m => m.Id == factory.MessageId(1), Ct)).CurrentAssigneeId);
            Assert.Equal(factory.UserId("admin"), (await db.Reviews.AsNoTracking().SingleAsync(r => r.MessageId == factory.MessageId(1) && r.Status == ORP.Domain.Reviews.ReviewStatus.InProgress, Ct)).ReviewerId);
        }
    }
    ...
    public async Task UndoComment_IsOptionalValidatedAndAudited_WithoutOverwritingApprovalComment(string? comment, string? expected)
    {
        ...
        var start = await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/start", new { level = 1 }, Ct);
        var reviewId = (await start.Content.ReadFromJsonAsync<StartReviewResponse>(Ct))!.ReviewId;
        (await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/approve", new { reviewId = await factory.LatestReviewIdAsync(factory.MessageId(1)), level = 1, comment = "Original approval" }, Ct)).EnsureSuccessStatusCode();
        Assert.Equal(HttpStatusCode.BadRequest, (await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/undo", new { reviewId, comment = new string('x', 2001) }, Ct)).StatusCode);
        using (var scope = factory.Services.CreateScope())
        {
            var db = scope.ServiceProvider.GetRequiredService<ORPDbContext>();
            Assert.Equal(MessageState.Completed, (await db.Messages.SingleAsync(m => m.Id == factory.MessageId(1), Ct)).State);
            Assert.False(await db.AuditEvents.AnyAsync(e => e.MessageId == factory.MessageId(1) && e.EventType == AuditEventType.ConfirmationUndone, Ct));
        }
        (await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/undo", new { reviewId, comment }, Ct)).EnsureSuccessStatusCode();
        var options = new JsonSerializerOptions(JsonSerializerDefaults.Web) { Converters = { new JsonStringEnumConverter() } };
        var trail = await admin.GetFromJsonAsync<PagedResult<AuditEventDto>>($"/api/messages/{factory.MessageId(1)}/audit", options, Ct);
    ...
    public async Task UndoGridAction_UsesLatestApproval_AndRequiresGlobalAdministrator()
    {
        using var admin = await AdministratorWithoutRoles();
        using var reviewer = Client("amelia.hart");
        (await admin.PutAsJsonAsync($"/api/admin/roles/{factory.RoleId("CS Reviewer")}/permissions", new { permissions = new[] { "message.view", "review.level1", "review.undo" } }, Ct)).EnsureSuccessStatusCode();
        (await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/assign", new { assignedTo = factory.UserId("amelia.hart") }, Ct)).EnsureSuccessStatusCode();
        var started = await reviewer.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/start", new { level = 1 }, Ct);
        var reviewId = (await started.Content.ReadFromJsonAsync<StartReviewResponse>(Ct))!.ReviewId;
        ...
        var grid = await admin.GetFromJsonAsync<JsonElement>(path, Ct);
        var row = grid.GetProperty("data").EnumerateArray().Single(r => r.GetProperty("id").GetInt64() == factory.MessageId(1));
        ...
        Assert.Equal(JsonValueKind.Null, ordinaryGrid.GetProperty("data").EnumerateArray().Single(r => r.GetProperty("id").GetInt64() == factory.MessageId(1)).GetProperty("undoReviewId").ValueKind);
        Assert.Equal(HttpStatusCode.Forbidden, (await reviewer.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/undo", new { reviewId }, Ct)).StatusCode);
        (await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/undo", new { reviewId }, Ct)).EnsureSuccessStatusCode();
        Assert.Equal(HttpStatusCode.Conflict, (await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/undo", new { reviewId }, Ct)).StatusCode);
        var refreshed = await admin.GetFromJsonAsync<JsonElement>(path, Ct);
        var updated = refreshed.GetProperty("data").EnumerateArray().Single(r => r.GetProperty("id").GetInt64() == factory.MessageId(1));
    ...
    public async Task UndoGridAction_IsUnavailableDuringReview_AndReopensLatestApprovedLevel()
    {
        ...
        (await admin.PostAsJsonAsync($"/api/messages/{id}/assign", new { assignedTo = factory.UserId("admin") }, Ct)).EnsureSuccessStatusCode();
        long latestId = 0;
    ...
    public async Task CanSelfAssignAndReviewAllLevels_ButCannotBypassWorkflowStates()
    {
        ...
        for (var level = 1; level <= 3; level++)
        {
            ...
            Assert.Equal(HttpStatusCode.Conflict, (await admin.PostAsJsonAsync($"/api/messages/{id}/reassign", new { assignedTo = factory.UserId("amelia.hart") }, Ct)).StatusCode);
            (await admin.PutAsJsonAsync($"/api/admin/users/{factory.UserId("admin")}/access", new { assignments = Array.Empty<object>() }, Ct)).EnsureSuccessStatusCode();
            (await admin.PostAsJsonAsync($"/api/messages/{id}/reviews/approve", new { reviewId = await factory.LatestReviewIdAsync(id), level }, Ct)).EnsureSuccessStatusCode();
        }
