...
public sealed class ReviewAttemptApiTests : IAsyncLifetime
{
    ...
    private async Task<long> Start(HttpClient client)
    {
        var response = await client.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/start", new { level = 1 }, Ct);
    ...
    public async Task OldDecision_CannotAffectRestartedAttempt(string action, bool undo, bool administrator)
    {
        ...
        (await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/{(undo ? "approve" : "cancel")}", new { level = 1, reviewId = oldId }, Ct)).EnsureSuccessStatusCode();
        if (undo)
        {
            (await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/undo", new { reviewId = oldId }, Ct)).EnsureSuccessStatusCode();
            (await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/assign", new { assignedTo = factory.UserId("amelia.hart") }, Ct)).EnsureSuccessStatusCode();
        }
        ...
        var response = await actor.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/{action}", new { level = 1, reviewId = oldId, comment = "Stale decision" }, Ct);
        ...
        (await actor.PostAsJsonAsync($"/api/messages/{factory.MessageId(1)}/reviews/{action}", new { level = 1, reviewId = newId }, Ct)).EnsureSuccessStatusCode();
    }
    ...
    public async Task Decision_RequiresValidIdMatchingMessageAndLevel(string action)
    {
        ...
        var path = $"/api/messages/{factory.MessageId(1)}/reviews/{action}";
        ...
        (await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(2)}/assign", new { assignedTo = factory.UserId("admin") }, Ct)).EnsureSuccessStatusCode();
        var other = await admin.PostAsJsonAsync($"/api/messages/{factory.MessageId(2)}/reviews/start", new { level = 1 }, Ct);
