...
{
    public StartReviewValidator() { RuleFor(x => x.Level).InclusiveBetween(1, 3); RuleFor(x => x.RowVersion).NotEmpty(); }
}
public sealed class ApproveReviewValidator : AbstractValidator<ApproveReviewRequest>
{
    public ApproveReviewValidator() { RuleFor(x => x.Level).InclusiveBetween(1, 3); RuleFor(x => x.RowVersion).NotEmpty(); RuleFor(x => x.Comment).MaximumLength(2000); }
}
public sealed class RejectReviewValidator : AbstractValidator<RejectReviewRequest>
{
    public RejectReviewValidator() { RuleFor(x => x.Level).InclusiveBetween(1, 3); RuleFor(x => x.RowVersion).NotEmpty(); RuleFor(x => x.Comment).NotEmpty().MaximumLength(2000); }
}
public sealed class UndoReviewValidator : AbstractValidator<UndoReviewRequest>
{
    public UndoReviewValidator() { RuleFor(x => x.ReviewId).GreaterThan(0); RuleFor(x => x.RowVersion).NotEmpty(); }
}
    ...
    ICurrentUser user, IClock clock, ICorrelationContext correlation)
{
    public async Task<long> HandleAsync(long messageId, StartReviewRequest request, CancellationToken cancellationToken)
    {
        ...
        var (message, workflow, reviews) = await LoadAsync(store, messageId, request.RowVersion, cancellationToken);
    ...
    internal static async Task<(Domain.Messages.Message Message, Domain.Workflows.WorkflowDefinition Workflow, List<Review> Reviews)> LoadAsync(
        ISwiftReviewStore store, long messageId, string rowVersion, CancellationToken cancellationToken)
    {
        ...
        store.SetExpectedRowVersion(message, Convert.FromBase64String(rowVersion));
    ...
    ICurrentUser user, IClock clock, ICorrelationContext correlation)
{
    ...
    {
        await validator.ValidateAndThrowAsync(request, cancellationToken);
        var (message, workflow, reviews) = await StartReviewHandler.LoadAsync(store, messageId, request.RowVersion, cancellationToken);
    ...
    ICurrentUser user, IClock clock, ICorrelationContext correlation)
{
    ...
    {
        await validator.ValidateAndThrowAsync(request, cancellationToken);
        var (message, _, reviews) = await StartReviewHandler.LoadAsync(store, messageId, request.RowVersion, cancellationToken);
        var review = reviews.SingleOrDefault(x => x.Level == request.Level && x.Status == ReviewStatus.InProgress)
            ?? throw new ResourceNotFoundException("Active review was not found.");
        var oldState = message.State;
